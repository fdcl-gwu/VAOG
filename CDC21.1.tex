...
It turns out that all of four cases of $p$ exhibit the similar order of long-term convergence, approximately $\mathcal{O}(k^{-2.3})$.
...
Furthermore, it is impressive that for the wide range of variations of step sizes presented in \Cref{fig:conv_h}.(b), the convergence in \Cref{fig:conv_h}.(a) are fairly consistent, which suggests the robustness of the LGVI against the choice of the step size. 
...
Three methods are considered, namely a splitting technique (SPLT)~\cite{tao2020variational}, a 4-th order fixed-step Runge-Kutta method (RK4), and a variable stepsize Runge-Kutta method (RK45) implemented by the Matlab \texttt{ode45} function with the tolerance of $10^{-8}$.
\TODO{ Molei: would you put a short description of the splitting method here.}
The goal of this comparison is not to claim that a certain method is superior than others.
But, it is to identify the numerical properties of LGVI compared with others. 
Having stated that, LGVI has the inherent drawback of implicit techniques.
Partially due to the fact that \eqref{eqn:Ek_SO3} is solved by a general purpose nonlinear equation solver, without developing any numerical scheme specialized to it, LGVI is substantially slower than the other explicit methods to the extent that the comparison is not meaningful. 

Instead, for the purpose of more interesting comparison, we exploit the property of LGVI providing consistent results for a wide range of step sizes, and we only utilize \eqref{eqn:muk_SO3} and \eqref{eqn:mukp_SO3} with a fixed prescribed step size. 
...
It is expected that RK4 requires more computation time as the gradient should be evaluated four times per a step, and it seems that the time adaptive algorithm of RK45 requries more frequent evaluations of the gradient. 
...
In particular, we consider variational accelerated methods based on the operating spliting, referred to as Lie-NAG-SC and Lie-NAG-C~\cite{tao2020variational}, which are conformally symplectic and group-structure preserving. 
\TODO{Molei: feel free add more descriptions about Lie-NAG here.}
...
On the other hand, Lie-NAG-SC demonstrates convergence rates beyond the polynomial order of $\mathcal{O}(t^{-p})$. 
ELGVI may benefit from early stopping if moderate accuracy is acceptable, as they enjoy the best initial convergence. 
\TODO{Molei: is there any explanation for the convergence of Lie-NAG-SC?}
...
\begin{figure}
    \centerline{
        \subfigure[convergence with respect to $t$]{\includegraphics[width=0.8\columnwidth]{comp_6b.eps}}
    }
    \centerline{
        \subfigure[orthogonality error of $R_k$]{\includegraphics[width=0.8\columnwidth]{comp_6c}}
    }
    \caption{Comparison with other discretization schemes for Bregman Euler-Lagrange equation}\label{fig:comp_disc}
\end{figure}
